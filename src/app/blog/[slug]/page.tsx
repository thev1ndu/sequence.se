...
import {
  ...
  getAllPosts,
  getPostBySlug,
} from "@/components/blog/data/posts";
...
export async function generateMetadata({
  ...
  const ogImage = image || `${siteConfig.url}/og/simple?title=${encodeURIComponent(title)}`;
...
function getPageJsonLd(post: Post): WithContext<PageSchema> {
  return {
    ...
    image:
      post.metadata.image ||
      `${siteConfig.url}/og/simple?title=${encodeURIComponent(post.metadata.title)}`,
    url: `${siteConfig.url}${getPostUrl(post)}`,
...
export default async function Page({
  ...
  return (
    <>
        ...
        <div className="flex items-center gap-2">
          ...
          {previous && (
            <Button variant="secondary" size="icon-sm" asChild>
              <Link href={`/blog/${previous.slug}`} prefetch={true}>
                <ArrowLeftIcon />
          ...
          {next && (
            <Button variant="secondary" size="icon-sm" asChild>
              <Link href={`/blog/${next.slug}`} prefetch={true}>
      ...
      <div suppressHydrationWarning={true} className="screen-line-before screen-line-after overflow-x-hidden">
        <div
          className={cn(
            "h-8",
            "before:absolute before:-left-[100vw] before:-z-1 before:h-full before:w-[200vw]",
            "before:bg-[repeating-linear-gradient(315deg,var(--pattern-foreground)_0,var(--pattern-foreground)_1px,transparent_0,transparent_50%)] before:[background-size:10px_10px] before:[--pattern-foreground:var(--color-edge)]/56"
          )}
        />
      </div>
